...
<body>
    ...
    <header>
        ...
        <nav class="navigation">
            <button onclick="location.href='formulari.php'" class="signup-button">Sell a Book</button>
    ...
    <div class="book-container">
        ...
        if (count($result) > 0) {
            foreach ($result as $row) {
                ...
                $car_name = htmlspecialchars($row['title']);
                $car_year = htmlspecialchars($row['author']);
                $car_image = htmlspecialchars($row['image']);

        
                $image_path = 'uploads/' . $book_image;
                ...
                echo "
                    <div class='book-box'>
                        <img src='$image_path' alt='Book Image'>
                        <h3>$title</h3>
                        <p>Author: $author</p>
                        <div class='actions'>
                            <button class='edit-button' onclick=\"location.href='edit_book.php?id=$book_id'\">Edit</button>
                            <button class='delete-button' onclick=\"confirmDelete($book_id)\">Delete</button>
                        </div>
                    </div>
                ";
            }
        } else {
            echo "<p class='no-data'>No books found for your account.</p>";
        }
        ?>
    </div>
